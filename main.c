...
int main (int argc, char **argv) {
	...
	FILE *f_data, *f_info;  //file handlers for data and information.
	
	//get our initial time
	time_start= clock();
	//check to see if we can write the files.
	f_info=fopen("simonsays.info", "w+");
	if(f_info==NULL){printf("file error\n");return(0);}  //leave if we cannot write
	f_data=fopen("simonsays.data", "wb+");
	if(f_data==NULL){printf("file error\n");return(0);}
	
	printf("filling a 4GiB file with 0x00s\n");
	ftruncate(fileno(f_data),(off_t)0x100000000);
	fflush(f_data);
	printf("done\n");
	time_tick= clock();
	time_elapsed = (time_tick - time_start) / CLOCKS_PER_SEC;
	printf("elapsed msec: %llu\n",time_elapsed);
...
*/
	...
	for(l_counter32=0;l_counter32<0xFFFFFFFF;l_counter32++)
	{
	   l_result=simon3264_encryption(a_keyexpansion,l_counter32);
	 //  fprintf(stdout,"0x%08x\n",l_result);
	   datafile_increment(f_data,l_result);
	...
	time_tick= clock();
    time_elapsed = (time_tick - time_start) / CLOCKS_PER_SEC;
	printf("data complete,elapsed msec: %llu\n",time_elapsed);
	fprintf(f_info,"data complete,elapsed msec: %llu\n",time_elapsed);
	//zeor the data array
	for(l_counter32=0;l_counter32<SIMON_T;l_counter32++)
	{
	    a_zerospectrum[l_counter32]=0;
	}

	
	 fprintf(f_info,"\nstarting spectrum search\n");
	//now count the spaces that have 0x00
	fseek(f_data,0,SEEK_SET);  //rest teh file
	for(l_counter32=0;l_counter32<0xFFFFFFFF;l_counter32++)
	{
	   fread(&l_singlebyte, 1, 1, f_data);  //get a single byte from the file.
	   //update the array
	   if(l_singlebyte<(SIMON_T-1))
	   {
	   		a_zerospectrum[l_singlebyte]=a_zerospectrum[l_singlebyte]+1;
	   }else
	   {
	   	  a_zerospectrum[(SIMON_T-1)]=a_zerospectrum[(SIMON_T-1)]+1;
	   }  
	}
	
	//now we update the info file
	for(l_counter32=0;l_counter32<SIMON_T;l_counter32++)
	{
		l_result=a_zerospectrum[l_counter32];
	    fprintf(f_info,"i[%02i] 0x%08x (%i)\n",l_counter32,l_result,l_result);
	}
	 time_tick= clock();
		time_elapsed = (time_tick - time_start) / CLOCKS_PER_SEC;
      printf("final time msec: %llu\n",time_elapsed);
      fprintf(f_info,"final time: %llu\n",time_elapsed);
